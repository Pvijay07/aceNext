...
import { Send, Sparkles, MessageSquare, RefreshCw, User, HelpCircle } from "lucide-react";
...
export default function TutorBot({ profile, courses, focusMode, onTrackXp }: TutorBotProps) {
  ...
  const sendMessage = async (textToSend: string) => {
    ...
    try {
      ...
      // Earn 10 XP per structured interaction
      onTrackXp(10);
    } catch (e) {
      console.error(e);
      setMessages((prev) => [
        ...prev,
        { role: "model", content: "Oops! I hit a routing buffer. Please check if your Express dev server has completed setup." }
      ]);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div id="ai-tutor-view" className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-[calc(100vh-14rem)] max-h-[750px]">
      {/* Left: Chat interface */}
      <div className={`${focusMode ? "lg:col-span-12 max-w-4xl mx-auto w-full" : "lg:col-span-8"} flex flex-col border border-slate-100 bg-white rounded-2xl shadow-sm overflow-hidden h-full`}>
        {/* Tutor Info bar */}
        <div className="bg-slate-900 text-white p-4 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h4 className="text-xs font-bold leading-none select-none">AceNext Dedicated AI Tutor</h4>
              <p className="text-[10px] text-slate-400 mt-1">Ready for doubt explanations • Earns +10 XP per reply</p>
            </div>
          </div>
        ...
        <div className="lg:col-span-4 flex flex-col justify-between h-full bg-slate-50 p-4 rounded-2xl border border-slate-100">
          <div className="space-y-4">
            <h4 className="font-extrabold text-[11px] text-slate-500 tracking-wider uppercase font-mono flex items-center gap-1">
              <HelpCircle className="h-3.5 w-3.5 text-slate-400" /> Continuous Topic Guide
            </h4>
            <p className="text-[11px] text-slate-500 leading-normal">
              Jumpstart your doubts using our specialized preset questions. Click any pill to initiate an immediate technical discussion with your tutor!
            </p>
          ...
          <div className="bg-gradient-to-tr from-indigo-50 to-blue-50 p-4 rounded-xl border border-indigo-100/50 space-y-2">
            <span className="text-[10px] font-extrabold text-indigo-700 tracking-widest uppercase font-mono">Dynamic Context</span>
            <p className="text-[11px] text-indigo-950 leading-relaxed font-sans">
              Your tutor tracks lesson progress in-stride. Complete syllabus sections to refresh and update your active AI scope.
            </p>
          </div>
        </div>
      )}
